...
const checkoutSchema = z.object({
  ...
  phone: z.string().trim().regex(/^[+0-9\s()\-]{8,30}$/, 'Invalid phone number'),
  address: z.string().trim().min(5, 'Address too short').max(200, 'Address too long'),
  city: z.string().trim().min(2, 'City too short').max(100, 'City too long'),
  province: z.string().trim().min(2, 'Province too short').max(100, 'Province too long'),
  postalCode: z.string().trim().min(3, 'Postal code too short').max(20, 'Postal code too long'),
  country: z.string().trim().min(2, 'Country too short').max(100, 'Country too long'),
});
...
const Checkout = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Validate all inputs with Zod before submission
      const validatedData = checkoutSchema.parse(shippingInfo);
      
      const fullAddress = `${validatedData.address}, ${validatedData.city}, ${validatedData.province} ${validatedData.postalCode}, ${validatedData.country}`;
